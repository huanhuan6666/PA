...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
		...
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1
			   ||										   // or
			   (sig_grs > 0x4 && exp < 0))			   // condition 2 is used in mul move right to make exp >= 0
		{
		...
		if (exp >= 0xff)//the exp overflow up
		{
	...
	else if (exp == 0 && (sig_grs >> (23 + 3)) == 1)//the 3rd condition
	{
	...
	if (!overflow)
	{
		...
		uint64_t grs = 0x7 & sig_grs;
		if(grs > 0x4)//carry one bit and test again
		{
		    sig_grs += 0x8;
		    if((sig_grs >> 26) > 1)//have a new carry
		       { 
		           sig_grs >>= 4;
		           exp++;
		           if(exp >= 0xFF)
		    ...
		    else//don't have a carry
		    {
		        sig_grs >>= 3;
		    }
		}
		else if(grs == 0x4)//at middle to be even
		{
            sig_grs >>= 3;
	        if((sig_grs & 0x1) == 1)//if the lsb is 1, add 1 
	            sig_grs += 0x1 ;
	        else//if the lsb is 0, do nothing
	            sig_grs = sig_grs;
	        if((sig_grs >> 23) > 1)//if make a carry
	            {
	                sig_grs >>= 1;
	                exp++;
	                if(exp >= 0xFF)
	    ...
	    else//if grs < 0x4, remove grs directly
	    {
	        sig_grs >>= 3;
	    }
	}
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	/* TODO: shift = ? */
	if(fb.exponent != 0 && fa.exponent != 0)
	    shift = fb.exponent - fa.exponent;
	else if(fb.exponent == fa.exponent)//all are zero
	    shift = 0;
	else// a is zero and b > 0
	...
	uint32_t exp_res = fb.exponent;//the bigger exp is the result's exp
...
uint32_t internal_float_mul(uint32_t b, uint32_t a )
{
	...
	if (fa.exponent == 0)//change a and b tobe guigehua
		fa.exponent++;
	if (fb.exponent == 0)
		fb.exponent++;
