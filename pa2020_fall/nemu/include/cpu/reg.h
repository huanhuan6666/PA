...
//the sreg define in PA 3-1 about SREG    
    typedef struct {
	    uint32_t limit :16;
	    uint32_t base :32;
    }GDTR;
    ...
    typedef struct {
	    // the 16-bit visible part, i.e., store the sreg selector's information
	    union {
		    uint16_t val;
		    struct {
			    uint32_t rpl :2;
	    ...
	    // the invisible part, i.e., cache part, store the sreg descriptor's information
	    struct {
		    uint32_t base;
		    uint32_t limit;
		    uint32_t type :5;
