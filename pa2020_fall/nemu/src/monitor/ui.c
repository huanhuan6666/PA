...
cmd_handler(cmd_x)
{
	...
	char *N = strtok(NULL, " ");//the N
	char *ad = strtok(NULL, " ");//the addr
	if (args != NULL)//the first arg is not empty
	{
	    int n = 1;
		if (sscanf(N, "%d", &n) == 1 && n > 0)//get the N in n
		{
			bool success;
		    uint32_t addr = expr(ad, &success);
		
		    printf("0x%x: ",addr);
		    for(int i = 0; i < n; i++)
		    {
			    printf(" 0x%08x", vaddr_read(addr, SREG_CS, 4));
			    addr += 4;
		    }
		    printf("\n");
		    fflush(stdout);
	...
	printf("usage: x N <expr>, where <expr> is a valid expr\n");
	printf("result: print N bytes of memory content starting from <addr>\n");
	fflush(stdout);
	return 0;
}
